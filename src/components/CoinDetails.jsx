...
const CoinDetails = () => {
  ...
  useEffect(() => {
    const fetchCoin = async () => {
      try {
        ...
        const chartResponse = await axios.get(`${server}/coins/${params.id}/market_chart?vs_currency=${currency}&days=${days}`);
        setCoin(response.data);
        setChartArray(chartResponse.data.prices)
        setLoading(false);
      } catch (error) {
        setError(true);
        setLoading(false);
  ...
  if (error) {
    return (
      <ErrorComponent message={"Some Error occured while Fetching the data for coin!!!!!!!"} />
    )
  }
